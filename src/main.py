...
if st.button("Host Lobby", use_container_width=True, type="primary"):#, disabled=not can_host):
    ...
    #     st.stop()
    st.error("You can't host a lobby since a host is already nearby!")
    # lobby_id = gen_code()
...
with st.form("join_form", border=True):
    ...
    u_name = st.text_input("Username", value=st.session_state.username_input, max_chars=15)
...
if submitted:
    if len(l_code) == 6:
        st.session_state.player_name = u_name.strip() or st.session_state.username_input
